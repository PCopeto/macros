...
TClonesArray *tofdData = new TClonesArray("R3BTofdPoint");
...
TH2F* RPCPosHit = new TH2F("RPCPosHit", "Hit Position in RPC;x;y", 500, -300, -100, 300, 120, -120);
TH2F* AvsZ2RPCPosHit = new TH2F("AvsZ2RPCPosHit", "Hit Position in RPC for A/Q 2;x;y", 500, -340, -200, 300, 120, -120);
TH2F* AvsZ1_5RPCPosHit = new TH2F("AvsZ1_5RPCPosHit", "Hit Position in RPC for A/Q 1.5;x;y", 500, -340, -200, 300, 120, -120);
TH2F* AvsZ1RPCPosHit = new TH2F("AvsZ1RPCPosHit", "Hit Position in RPC for A/Q 1;x;y", 500, -340, -200, 300, 120, -120);
...
TH1F* DeltaX_Multi2 = new TH1F("DeltaX_Multi2", "delta X for multi 2", 300, 120, -120);
TH2F* tofd_Z_vs_AoZ = new TH2F("tofd_Z_vs_AoZ", "ToFD Z vs A/Z;A/Z;Z", 100, 1, 4, 120, 0, 10);
TH2F* RPCandTofd_Z_vs_AoZ = new TH2F("RPCandTofd_Z_vs_AoZ", "Both RPC and ToFD Z vs A/Z;A/Z;Z", 100, 1, 4, 120, 0, 10);
TH2F* tofd_HeORD_RPC = new TH2F("tofd_HeORD_RPC", "Z vs A/Z in ToFD when He or D in RPC;A/Z;Z", 100, 1, 4, 120, 0, 10);
TH2F* RPCnotTofd = new TH2F("RPCnotTofd", "Z vs A/Z in RPC when no hits in ToFD;A/Z;Z", 100, 1, 4, 120, 0, 10);
TH2F* GLAD_Neutrons = new TH2F("GLAD_neutrons", "Netron Multi in GLAD;Neutron_Multi;GLAD_Multi", 10, 0, 10, 10, 0, 10);
TH2F* CALIFA_Neutrons = new TH2F("CALIFA_neutrons", "Netron Multi in CALIFA;Neutron_Multi;CALIFA_Multi", 10, 0, 10, 10, 0, 10);
TH2F* Lost_Neutrons = new TH2F("Lost_neutrons", "Netron Multi Lost;Neutron_Multi;Lost_Multi", 10, 0, 10, 10, 0, 10);
TH3F* Neutrons = new TH3F("Neutrons", "Number of neutrons in each detector;Neutron_Multi;GLAD;CALIFA",10,0,10,10,0,10,10,0,10);
TH2F* HeOrD_RPC_CorN_ToFD_GLAD = new TH2F("HeOrD_RPC_CorN_ToFD_GLAD", "GLAD Neutrons (He/D in RPC, C/N in ToFD);Neutron_Multi;GLAD_Multi", 10, 0, 10, 10, 0, 10);
TH2F* HeOrD_RPC_CorN_ToFD_CALIFA = new TH2F("HeOrD_RPC_CorN_ToFD_CALIFA", "CALIFA Neutrons (He/D in RPC, C/N in ToFD);Neutron_Multi;CALIFA_Multi", 10, 0, 10, 10, 0, 10);
TH2F* HeOrD_RPC_CorN_ToFD_Lost = new TH2F("HeOrD_RPC_CorN_ToFD_Lost", "Lost Neutrons (He/D in RPC, C/N in ToFD);Neutron_Multi;Lost_Multi", 10, 0, 10, 10, 0, 10);
...
void real_RPC_Analysis(){
  ...
  std::vector<double> PosX_Multi2;
  std::vector<double> frag_A_Multi1_RPCandTofd_vec;
  std::vector<double> frag_Z_Multi1_RPCandTofd_vec;
  std::vector<double> neutron_theta_deg_vec;


  TString output_filename_hit = TString::Format("real_RpcHits.root");
  ...
  outtree->Branch("frag_A_Multi1_RPCandTofd", &frag_A_Multi1_RPCandTofd_vec);
  outtree->Branch("frag_Z_Multi1_RPCandTofd", &frag_Z_Multi1_RPCandTofd_vec);
  outtree->Branch("neutron_theta", &neutron_theta_deg_vec);
  ...
  fileList.push_back("real_sim.root");
  ...
  for(int s = 0 ; s < fileList.size() ; s++){
    ...
    TBranch *branchTofdData = eventTree->GetBranch("TofDPoint");
    branchTofdData->SetAddress(&tofdData);

    TRandom rand(time(0));

    int nMCtracks = 0, nRpc_point = 0, nTofd_point = 0, mass_frag = 0; //, nNeuland_point =0, nTra_point;
...
//    int counter_tracks = 0, counter_frags = 0, counter_10Be = 0, counter_proton = 0;
    ...
    for(int t = 0; t< nev;t++){
      ...
      std::unordered_set<int> tofdTrackSet;
      std::unordered_set<int> rpcTrackSet;

      neutron_theta_deg_vec.clear();
...
//      num_part = fParticles;
      ...
      nTofd_point = tofdData->GetEntriesFast();
      //nTofd_point = tofdData->GetEntries();
...
//      TLorentzVector p_temp, n_temp;
      ...
      for (Int_t j = 0; j < nMCtracks; j++) {
        ...
        if (track->GetMotherId() == -1) {  // Only consider particles with mother ID -1
          ...
          //cout << " ################### " << track->GetPdgCode() << " " << track->GetPx() << " " << track->GetPy() << " " << track->GetPz() << endl;
          if (track->GetPdgCode() > 1000010010) {
            fZ_in = int(track->GetPdgCode() / 10000) - 100000;
            fA_in = 0.1 * (track->GetPdgCode() - (100000 + fZ_in) * 10000);
          }
          if (track->GetPdgCode() == 2212) {  // Proton case
            fZ_in = 1;
            fA_in = 1;
          }
          if (track->GetPdgCode() == 2112) {  // Neutron case
            fZ_in = 1;
            fA_in = 0;

            n_neutrons +=1;

            p_neutron.SetXYZM(track->GetPx(),track->GetPy(),track->GetPz(),mass_frag);
            double theta = p_neutron.Theta(); // theta in radians
            double theta_deg = theta * (180.0 / TMath::Pi());

            neutron_theta_deg_vec.push_back(theta_deg);
      ...
      //END OF MCTRACKS LOOP
      
      GLAD_Neutrons->Fill(n_neutrons,GLAD_neutrons);
      CALIFA_Neutrons->Fill(n_neutrons,CALIFA_neutrons);
      Lost_Neutrons->Fill(n_neutrons,Lost_neutrons);
      Neutrons->Fill(n_neutrons,GLAD_neutrons,CALIFA_neutrons);


      bool Tofd_Flag = false;
      bool hasC_or_N_in_ToFD = false;

      for (Int_t l = 0; l<nTofd_point; l++){
        auto tofdPoint = dynamic_cast<R3BTofdPoint*>(tofdData->At(l));

        Int_t tofdTrackId = tofdPoint->GetTrackID();

        //tofdTrackSet.insert(tofdTrackId);

        if (trackIdToAzMap.find(tofdTrackId) != trackIdToAzMap.end()){
          tofdTrackSet.insert(tofdTrackId);
          Tofd_Flag = true;
          const auto& [A_tofd, Z_tofd] = trackIdToAzMap[tofdTrackId];
          tofd_Z_vs_AoZ->Fill(A_tofd/Z_tofd, Z_tofd);

          if (Z_tofd == 6 || Z_tofd == 7) {
            hasC_or_N_in_ToFD = true;
          }
        }
      }
      ...
      for (Int_t k = 0; k < nRpc_point; k++) {
        ...
        if (trackIdToAzMap.find(rpcTrackId) != trackIdToAzMap.end()) {
          ...
          if ((A/Z < 2) && (A/Z > 1)) AvsZ1_5RPCPosHit->Fill(rpc_x,rpc_y);
          if (A/Z==1) AvsZ1RPCPosHit->Fill(rpc_x,rpc_y);

          //cout << A << " " << Z << rpcTrackId << " " << rpc_pos.X() << " " << rpc_pos.Y() << " " << rpc_pos.Z() << endl;

          PosY_Multi2.push_back(rpc_y);
          PosX_Multi2.push_back(rpc_x);

          //frag_A_vec.clear();
          //frag_Z_vec.clear();
          //frag_A_Multi1_vec.clear();
          //frag_Z_Multi1_vec.clear();
          //frag_A_Multi1_RPCandTofd_vec.clear();
          //frag_Z_Multi1_RPCandTofd_vec.clear();
...
*/
          if (Z==2 || (A==2 && Z==1)){
            ...
            for (Int_t l = 0; l<nTofd_point; l++){
              ...
              if (trackIdToAzMap.find(tofdTrackId) != trackIdToAzMap.end()){
                const auto& [A_tofd, Z_tofd] = trackIdToAzMap[tofdTrackId];
                tofd_HeORD_RPC->Fill(A_tofd/Z_tofd, Z_tofd);
              }
            }
          }

          //if (Z==2 && A==4) alpha_multi +=1;

          if (!Tofd_Flag){
            RPCnotTofd->Fill(A/Z,Z);
          }
/*
...
*/            
      ...
      //if (alpha_multi==1) alpha_event_multi +=1;

      for (const auto& trackId : rpcTrackSet) {
        if (tofdTrackSet.count(trackId) && trackIdToAzMap.count(trackId)) {
          double A = trackIdToAzMap[trackId].first;
          double Z = trackIdToAzMap[trackId].second;

          RPCandTofd_Z_vs_AoZ->Fill(A/Z, Z);
        }
      }
      ...
      if (multi == 1){
        ZvsAoZMulti1RPC->Fill(RPC_A[0]/RPC_Z[0], RPC_Z[0]);
/*        if (RPC_Z[0] == 2){
          std::cout<<"He or D in event: "<< t << endl;
        }*/
        for (const auto& entry : trackIdToAzMap) {
          frag_A_Multi1_vec.push_back(entry.second.first);
          frag_Z_Multi1_vec.push_back(entry.second.second);
      ...
      if ((multi == 1) && Tofd_Flag){
        for (const auto& entry : trackIdToAzMap){
          frag_A_Multi1_RPCandTofd_vec.push_back(entry.second.first);
          frag_Z_Multi1_RPCandTofd_vec.push_back(entry.second.second);
        }
        //if (alpha_multi==1) alpha_event_multi +=1;
      }
      ...
      if (multi == 2){
        ...
        if (PosX_Multi2.size() != 2){
          std::cout << "Error in PosX_Multi2 vector!" << endl;
        }
        else{
          /*if(PosY_Multi2[0] - PosY_Multi2[1] == 0 || PosX_Multi2[0] - PosX_Multi2[1] == 0){
            cout << "||||||||||||||||||||||||||||||||| "<< PosY_Multi2[0] << " " << PosY_Multi2[1] << " " << PosX_Multi2[0] << " " << PosX_Multi2[1] << endl;
            cout << "||||||||||||||||||||||||||||||||| "<< PosY_Multi2[0] - PosY_Multi2[1] << " " << PosX_Multi2[0] - PosX_Multi2[1] << endl;
          }*/
          DeltaY_Multi2->Fill(PosY_Multi2[0] - PosY_Multi2[1]);
          DeltaX_Multi2->Fill(PosX_Multi2[0] - PosX_Multi2[1]);
        }
      }
      ...
      if (hasHeOrD_in_RPC && hasC_or_N_in_ToFD) {
        HeOrD_RPC_CorN_ToFD_GLAD->Fill(n_neutrons, GLAD_neutrons);
        HeOrD_RPC_CorN_ToFD_CALIFA->Fill(n_neutrons, CALIFA_neutrons);
        HeOrD_RPC_CorN_ToFD_Lost->Fill(n_neutrons, Lost_neutrons);
      }
    ...
    DeltaX_Multi2->Write();
    tofd_Z_vs_AoZ->Write();
    RPCandTofd_Z_vs_AoZ->Write();
    tofd_HeORD_RPC->Write();
    RPCnotTofd->Write();
    AvsZ2RPCPosHit->Write();
    AvsZ1_5RPCPosHit->Write();
    AvsZ1RPCPosHit->Write();
    outtree->Write();

    GLAD_Neutrons->Write();
    CALIFA_Neutrons->Write();
    Lost_Neutrons->Write();
    Neutrons->Write();
    //std::cout << "Neutrons in GLAD: " << GLAD_neutrons << "; Neutrons in CALIFA: " << CALIFA_neutrons << "; Lost neutrons: " << Lost_neutrons << endl;
    //std::cout << "Number of (p,pa) events: " << alpha_event_multi << endl;
    delete _file2;
